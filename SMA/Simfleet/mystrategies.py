...
from simfleet.helpers import PathRequestException, are_close
            ...
            if performative == REQUEST_PERFORMATIVE:
                if self.agent.status == TRANSPORT_WAITING:
                    if not self.has_enough_autonomy(content["origin"], content["dest"]):
                        ...
                        """if are_close(content["origin"],self.agent.get_position(), 2000):"""
                        await self.send_proposal(content["customer_id"], {})
                        """El transporte no se espera al rechazo para seguir mandando propuestas"""
                ...
                logger.debug("!!!!Transport {} got refusal from customer".format(content["customer_id"]))

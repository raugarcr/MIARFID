...
class MyTransportStrategy(TransportStrategyBehaviour):
    ...
    async def on_start(self):
       ...
       self.agent.set("temporizador",time.time())
       self.agent.set("listaClientes",{})
       print("Hola")
    ...
    async def run(self):
            ...
            if performative == REQUEST_PERFORMATIVE:
                if self.agent.status == TRANSPORT_WAITING:
                    if not self.has_enough_autonomy(content["origin"], content["dest"]):
                        ...
                        lista = self.agent.get("listaClientes")
                        distancia = distance_in_meters(self.agent.get_position(), content["origin"])
                        lista[content["customer_id"]] = distancia
                        self.agent.set("listaClientes", lista)
        ...
        if self.agent.status == TRANSPORT_WAITING and (time.time() - self.agent.get("temporizador")) > 1 and len(self.agent.get("listaClientes").keys()) > 0:
            lista = self.agent.get("listaClientes")
            listaOrdenada = {cliente: distancia for cliente, distancia in sorted(lista.items(), key=lambda item: item[1])}
            print(time.time() - self.agent.get("temporizador"))
            await self.send_proposal(list(listaOrdenada.keys())[0])
            self.agent.status = TRANSPORT_WAITING_FOR_APPROVAL
            del listaOrdenada[list(listaOrdenada.keys())[0]]
            self.agent.set("listaClientes", listaOrdenada)
################################################################
#                                                              #
#                       Customer Strategy                      #
#                                                              #

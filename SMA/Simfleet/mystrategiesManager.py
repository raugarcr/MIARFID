...
class MyFleetManagerStrategy(FleetManagerStrategyBehaviour):
    ...
    async def on_start(self):
        ...
        self.agent.set("tablaTransportes", {})
    ...
    async def run(self):
        ...
        """Construimos un listado con los transportes registrados"""
        idsTransportes = [str(transporte["jid"]) for transporte in self.get_transport_agents().values()]
        if msg:
            """Diferenciamos mensajes de transportes y clientes"""
            if str(msg.sender) in idsTransportes:
                """Registramos la posicion de los transportes"""
                contenido = json.loads(msg.body)
                listadoTransp = self.agent.get("tablaTransportes")
                listadoTransp[contenido["transport_id"]] = contenido["posicion"]
                self.agent.set("tablaTransportes", listadoTransp)
            else:
                """Con la posicion del cliente calculamos la distancia a los transportes y enviamos las solicitudes en orden"""
                contenido = json.loads(msg.body)
                posicionCust = contenido["origin"]
                listadoTransp = self.agent.get("tablaTransportes")
                listaTranspDist = {}
                for transportId in list(listadoTransp.keys()):
                    distancia = distance_in_meters(posicionCust,listadoTransp[transportId])
                    listaTranspDist[transportId] = distancia
                listaOrdenada = {transporte: distancia for transporte, distancia in sorted(listaTranspDist.items(), key=lambda item: item[1])}
                for transportId in list(listaOrdenada.keys()):
                    msg.to = transportId
                    logger.debug("Manager sent request to transport {}".format(transportId))
                    await self.send(msg)   
                """
...
class MyTransportStrategy(TransportStrategyBehaviour):
    ...
    async def on_start(self):
        await super().on_start()
        self.agent.set("lastPosition", self.agent.get_position())
        content = {
            "transport_id": str(self.agent.jid),
            "posicion": self.agent.get_position()
        }
        msg = Message()
